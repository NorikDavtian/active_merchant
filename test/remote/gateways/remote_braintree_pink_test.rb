...
class RemoteBraintreePinkTest < Test::Unit::TestCase
  def setup
    @gateway = BraintreeGateway.new(fixtures(:braintree_Pink))
  ...
  def test_successful_purchase_with_hold_in_escrow
    @options.merge({:merchant_account_id => fixtures(:braintree_Pink)[:merchant_account_id], :hold_in_escrow => true})
    assert response = @gateway.authorize(@amount, @credit_card, @options)
    assert_success response
    assert_equal '1000 Approved', response.message
  end
  ...
  def test_invalid_login
    gateway = BraintreePinkGateway.new(:merchant_id => "invalid", :public_key => "invalid", :private_key => "invalid")
    assert response = gateway.purchase(@amount, @credit_card, @options)
    assert_failure response
  ...
  def test_successful_credit_with_merchant_account_id
    assert response = @gateway.credit(@amount, @credit_card, :merchant_account_id => fixtures(:braintree_Pink)[:merchant_account_id])
  ...
  def test_successful_authorize_with_merchant_account_id
    assert response = @gateway.authorize(@amount, @credit_card, :merchant_account_id => fixtures(:braintree_Pink)[:merchant_account_id])
    assert_success response, "You must specify a valid :merchant_account_id key in your fixtures.yml for this to pass."
    assert_equal '1000 Approved', response.message
    assert_equal fixtures(:braintree_Pink)[:merchant_account_id], response.params["braintree_transaction"]["merchant_account_id"]
  end
  ...
  def test_successful_validate_on_store_with_verification_merchant_account
    card = credit_card('4111111111111111', :verification_value => '101')
    assert response = @gateway.store(card, :verify_card => true, :verification_merchant_account_id => fixtures(:braintree_Pink)[:merchant_account_id])
    assert_success response, "You must specify a valid :merchant_account_id key in your fixtures.yml for this to pass."
    assert_equal 'OK', response.message
  end
